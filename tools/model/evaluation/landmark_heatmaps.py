...
def main(argv):
    ...
    source_patches_placeholder = tf.squeeze(tf.map_fn(lambda x: get_patch_at(x, source_image_region, args.patch_size), coords_placeholder, parallel_iterations=8, back_prop=False))
    ...
    with tf.Session(config=config).as_default() as sess:
        ...
        with open(args.output + "_" + str(region_size) + ".csv",'wt') as outfile:
            fp = csv.DictWriter(outfile, ["landmark", "min_idx", "min_idx_value", "rank", "landmark_value"])
            fp.writeheader()
            ...
            for k in range(len(source_landmarks)):
                ...
                heatmap = np.ndarray((region_size, region_size))
                ...
                for i in range(num_splits):
                    ...
                    similarity_values = sess.run(similarities_bd,feed_dict=feed_dict)
                    #heatmap.extend(similarity_values)
                    for idx, val in zip(batch_coords, similarity_values):
                        heatmap[idx[0] - offset, idx[1] - offset] = val
                ...
                fp.writerow({"landmark": k, "min_idx": min_idx, "min_idx_value": heatmap[min_idx[0], min_idx[1]],"rank": rank , "landmark_value": landmark_value})
                outfile.flush()

                print(min_idx, rank)
                matplotlib.image.imsave(args.output + "_" + str(region_size)+ "_" + str(k) + ".jpeg", heatmap, cmap='plasma')
